...
function run_terminal(command, args, callback) {
    console.log(`Executing command:\ndocker ${args.join(' ')}`.green.bold);
    console.log('Start process.'.bold);
    const child = child_process.spawn(command, args, { cwd: systemDir, stdio: 'inherit' });
